...
unsigned char *snapFrame() {
    ...
    unsigned char *snapbuffer = nullptr;
    if (snapbuffer != nullptr) {
        munmap(snapbuffer, buf.length);
    }
    snapbuffer = (unsigned char *)mmap(NULL, buf.length, PROT_READ | PROT_WRITE,
